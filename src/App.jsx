...
function getOptimalTileSize() {
    ...
    if (isMobile) {
        ...
        if (memoryInfo >= 8 && maxTextureSize >= 8192) return 128;
        if (memoryInfo >= 4 && maxTextureSize >= 4096) return 96;
        return 64;
    }
}
...
}You're in the final stretch, and this last bug is a classic off-by-one error. The black line is appearing because of a tiny miscalculation in the logic that handles the very last row of tiles in each worker strip, causing a gap.

Your code is incredibly advanced now, with dynamic tile sizing, caching, and reflection padding—excellent work! We just need to fix this one last piece of math.

---
### ## The Problem: A Flaw in the "Last Tile" Logic

Inside your web worker, the complex `if/else if` chain that calculates the `copyWidth` and `copyHeight` for stitching has a small error. The conditions for `isLastCol` and `isLastRow` are slightly off, causing the dimensions of the final tiles in each row/column to be miscalculated. This creates a situation where the last drawn piece doesn't quite meet the edge, leaving a black line.

---
### ## The Solution: Simplified and Corrected Stitching Math

I've replaced the entire complex `if/else if` block with a much simpler and more robust calculation. This new logic correctly determines the dimensions of the piece to copy from each tile (the "source") and where to place it (the "destination"), ensuring that the first, middle, and last tiles all align perfectly without gaps.

This change is only within the Web Worker code. The rest of your app's logic is sound.

---
### ## The Final Corrected Code

Here is the complete code for your `App` component with the corrected stitching logic. This should eliminate the final black line artifact and give you a perfect, seamless result.
